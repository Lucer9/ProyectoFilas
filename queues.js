...
const mms = (a, u, s, n) => {
  ...
  if(n<=s){
    const pn=((Math.pow(a/u,n))/factorial(n))*p0
  }else{
    const pn=((Math.pow(a/u,n))/(factorial(k)) *(Math.pow(s,n-s)))*p0
  }
  
  //Tiempo promedio de unidades en cola
  const lq = ((Math.pow(a/u,s)*(u*a))/(factorial(s-1)*Math.pow((s*u-a),2)))*p0
